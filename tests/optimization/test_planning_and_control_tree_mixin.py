...
class Model(
    PlanningMixin, ControlTreeMixin, ModelicaMixin, CollocatedIntegratedOptimizationProblem
):
...
class TestPlanningAndControlTreeMixin(TestCase):
    ...
    def test_planning_variables(self):
        ...
        self.assertTrue(self.results[1]["u"][0] == self.results[2]["u"][0])

        self.assertTrue(np.any(self.results[1]["u"] != self.results[2]["u"]))
