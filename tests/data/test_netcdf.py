...
filepaths = {0: 'timeseries_import_ensemblesize1_noRealization',
             1: 'timeseries_import_ensemblesize1_withRealization',
             2: 'timeseries_import_ensemble'}

file_associations = {'test_inputfiles': list(range(3)),
...
def create_expectation_list(files, expected_values):
    return {key_to_file(_file): _expected_value for (_file, _expected_value)
            in zip(files, expected_values)}
        ...
        if self._variables[rl]:
            ...
            for _variable, _variable_alias in zip(self._variables[rl], aliases_of_interest):
                if _variable != 'ensemble':
                    self.assertTrue(self.variable_found[_variable], '''Variable '{}'
 was not found by its alias '{}' in file '{}.nc' .'''.format(_variable, _variable_alias, filepaths[file_key]))
                ...
                if self._variable_associated_attribute_requirements[rl]:
                    if self._variable_associated_attribute_requirements[rl][_variable]:
                        for _attribute, _attribute_requirement in \
                                self._variable_associated_attribute_requirements[rl][_variable].items():
                            self.assertTrue(_attribute in self.ncdf_var[_variable].ncattrs()
                                            and getattr(self.ncdf_var[_variable], _attribute) == _attribute_requirement,
                                            '''Variable '{}' with alias '{}' in '{}.nc' is missing attribute '{}'
 with value '{}'.'''.format(_variable, _variable_alias, filepaths[file_key], _attribute, _attribute_requirement))
                    ...
                    if self._variable_associated_dimensions[rl][_variable]:
                        if self.variable_found[_variable]:
                            for _dimension in self._variable_associated_dimensions[rl][_variable]:
                                self.assertIn(_dimension, self.ncdf_var[_variable].dimensions, '''
...
class TestImportDataset(TestCase):
    ...
    def test_get_ensemble_size(self):
        files = ('timeseries_import_ensemblesize1_noRealization',
    ...
    def test_read_times(self):
        ...
        expected_datetimes = [[forecast_datetime + timedelta(minutes=i) for i in range(25)], ]*3
    ...
    def test_find_timeseries_variables(self):
        files = ('timeseries_import_ensemblesize1_noRealization',
                 'timeseries_import_ensemblesize1_withRealization',
                 'timeseries_import_ensemble')
        expected_variables = [['u', 'x', 'u_min', 'u_max', 'w', 'constant_input'], ]*3
    ...
    def test_stations(self):
        ...
        self.expected_station_attributes = create_expectation_list(files, [['lon', 'lat'], ]*3)
    ...
    def test_read_timeseries_values(self):
        ...
        self.expected_x_values['loc_b'] = create_expectation_list(files,
                                                                  [deepcopy(x_var_s), ]*2 + [deepcopy(x_var_e)])
        x_var_s[0, 0] = 1.02
        x_var_s[2, 0] = 0.03
        x_var_e[0, 0, 0] = 1.02
        x_var_e[2, 0, 0] = 0.03
        self.expected_x_values['loc_a'] = create_expectation_list(files,
                                                                  [deepcopy(x_var_s), ]*2 + [deepcopy(x_var_e)])
    ...
    def _subtest_read_times(self, file_key):
        ...
        time_diffs = [(x-y).total_seconds() for (x, y) in zip(datetimes, self.expected_datetimes[file_key])]
    ...
    def _subtest_stations(self, file_key):
        ...
        for id in ids:
            ...
            self.assertEqual(len(read_attributes), self.expected_station_attributes_lengths[file_key])
    ...
    def _subtest_read_timeseries_values(self, file_key):
        ...
        for station_id in self.expected_x_values.keys():
            ...
            for ensemble_member in range(ensemble_size):
                ...
                if ensemble_size == 1:
                    self.assertSequenceEqual(self.expected_x_values[station_id][file_key][:, station_index].tolist(),
                                             values.tolist())
                else:
                    self.assertSequenceEqual(
                        self.expected_x_values[station_id][file_key][:, station_index][:, ensemble_member].tolist(),
                        values.tolist())
...
class TestExportDataset(TestCase):
 ...
 of variable '{}' does not match its expected value.'''.format(_attribute, _variable))
    ...
    def check_associated_dimensions(self, _variables, _expected_variable_associated_dimensions):
        for (_variable, _expected_dimensions) in zip(_variables, _expected_variable_associated_dimensions):
            _variable_associated_dimensions = self.dataset.variables[_variable].dimensions
            self.assertEqual(set(_variable_associated_dimensions), set(_expected_dimensions), '''
The dimensions of variable '{}' are not as expected.'''.format(_variable))
    ...
    def test_global_attributes(self):
        ...
        self.assertIn('history', self.dataset._ExportDataset__dataset.ncattrs(),
    ...
    def test_write_times(self):
        ...
        self.assertTrue(self.dataset.dimensions['time'].isunlimited(),
    ...
    def test_write_station_data(self):
        ...
        for _dimension in ('station', 'char_leng_id'):
            self.assertIn(_dimension, self.dataset.dimensions, '''Dimension '{}'
 is missing, but should be available to be able to identify the stations.'''.format(_dimension))
        ...
        for _variable in ('station_id', 'lat', 'lon'):
            self.assertIn(_variable, self.dataset.variables, '''Variable '{}' is missing,
 but should be available to be able to identify the stations.'''.format(_variable))
        ...
        _attribute_values_lon = ('longtitude', 'Station coordinates, longtitude', 'degrees_east', 'X')
    ...
    def test_create_variables_without_ensemble(self):
        ...
        self.import_dataset = netcdf.ImportDataset(data_path(), 'timeseries_import_ensemblesize1_noRealization')
        stations = self.import_dataset.read_station_data()
    ...
    def test_create_variables_with_ensemble(self):
        ...
        unique_parameter_ids = ['u_min', 'u_max', 'x', 'w', 'constant_input', 'constant_output', 'u']
        ensemble_size = 3
        initial_time = 0.
        reference_datetime = datetime(1970, 1, 1, 0, 0)
        times = [x*60 for x in range(25)]
    ...
    def test_write_output_values(self):
        ...
        x_var_e = np.zeros((25, 2, 3))
        self.expected_x_values = {}
        self.expected_x_values['loc_b'] = create_expectation_list(files, [deepcopy(x_var_s), ]*2
                                                                  + [deepcopy(x_var_e)])
        x_var_s[0, 0] = 1.02
        x_var_s[2, 0] = 0.03
        x_var_e[0, 0, 0] = 1.02
        x_var_e[2, 0, 0] = 0.03
        self.expected_x_values['loc_a'] = create_expectation_list(files, [deepcopy(x_var_s), ]*2
                                                                  + [deepcopy(x_var_e)])
    ...
    def _subtest_write_output_values(self, file_key):
            ...
            for _variable in unique_parameter_ids:
                ...
                for ensemble_member in range(ensemble_size):
                    variables[station_id][_variable][ensemble_member] = \
                        self.import_dataset.read_timeseries_values(list(stations.station_ids).index(station_id),
                                                                   _variable, ensemble_member)
        ...
        for station_id in stations.station_ids:
            for _variable in unique_parameter_ids:
                for ensemble_member in range(ensemble_size):
                    self.dataset.write_output_values(station_id, _variable, ensemble_member,
                                                     variables[station_id][_variable][ensemble_member], ensemble_size)
        ...
        for station_id in self.expected_x_values.keys():
            ...
            for ensemble_member in range(ensemble_size):
                ...
                if ensemble_size == 1:
                    ...
                    self.assertSequenceEqual(self.expected_x_values[station_id][file_key][:, station_index]
                                             [:, ensemble_member].tolist(), values.tolist())
